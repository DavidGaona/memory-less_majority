...
case class BuildCustomNetwork(agents: Array[AgentInitialState])
...
class Monitor extends Actor {
  ...
  def receive: Receive = {
    case AddNetworks(numberOfNetworks, numberOfAgents, density, degreeDistribution) =>
      ...
      for (i <- 0 until numberOfNetworks) {
        val network = createNetwork(numberOfAgents, density, degreeDistribution, dataSavingPath, s"Agent${i + 1}")
        networks = networks :+ network
      }
      self ! RunBatch

    case AddSpecificNetwork(agents, name) =>
      val network = createNetwork(agents.length, -1, 2.5, dataSavingPath, name)
      networks = networks :+ network
  ...
  def createNetwork(numberOfAgents: Int, density: Int, degreeDistributionParameter: Double, dataSavingPath: String,
                    name: String): ActorRef = {
    context.actorOf(Props(new Network(numberOfAgents, density, degreeDistributionParameter, dataSavingPath)), name)
  }
